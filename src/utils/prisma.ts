...
export async function getUserById(userId: string, client: PrismaClient, includeField?: string): Promise<User> {
    try {
...
export async function getListingById(listingId: string, client: PrismaClient, includeField?: string): Promise<Listing> {
    try {
        ...
        if (!listing) throw new NotFoundError(`Could not find listing, id: ${listingId}`)
...
export async function getBookingById(bookingId: string, client: PrismaClient, includeField?: string): Promise<Booking> {
    try {
        ...
        if (!booking) throw new NotFoundError(`Could not find bookng, id: ${bookingId}`)
